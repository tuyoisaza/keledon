...
exports.autoBrowseExecutor = exports.AutoBrowseExecutor = void 0;
const playwright_core_1 = require("playwright-core");
class AutoBrowseExecutor {
    constructor() {
        this.browser = null;
        this.page = null;
     ...
     */
    async connectOverCDP(config) {
        if (this.isInitialized && this.isCDPMode) {
            return;
        }
        try {
            this.browser = await playwright_core_1.chromium.connectOverCDP(config.cdpUrl, {
     ...
     */
    async executeGoal(goal, executionContext) {
    ...
    planSalesforce(objective, targetUrl) {
        const steps = [];
        if (targetUrl) {
        ...
        if (objective.includes('case') || objective.includes('caso')) {
            steps.push({ action: 'wait_for_selector', target: '[title="Cases"]', description: 'Wait for Cases tab' }, { action: 'click', target: '[title="Cases"]', description: 'Click Cases tab' }, { action: 'wait_for_load', description: 'Wait for page load' });
            if (objective.includes('search') || objective.includes('buscar')) {
                steps.push({ action: 'wait_for_selector', target: '.forceSearchInput', description: 'Wait for search box' }, { action: 'read_content', description: 'Read cases list' });
            }
        }
        if (objective.includes('contact') || objective.includes('cliente')) {
            steps.push({ action: 'wait_for_selector', target: '[title="Contacts"]', description: 'Wait for Contacts tab' }, { action: 'click', target: '[title="Contacts"]', description: 'Click Contacts tab' });
        }
        steps.push({ action: 'capture_screenshot', description: 'Capture final state' });
        return steps;
    }
    planGenesys(objective) {
        return [
    ...
    planGeneric(objective, targetUrl) {
        const steps = [];
        if (targetUrl) {
            steps.push({ action: 'navigate', target: targetUrl, description: 'Navigate to target' });
        }
        steps.push({ action: 'wait_for_load', description: 'Wait for page load' }, { action: 'read_content', description: 'Read page content' }, { action: 'capture_screenshot', description: 'Capture current state' });
        return steps;
    }
    async executeStep(page, step, index, total) {
    ...
    shouldStopExecution(stepResult, goal) {
    ...
    determineGoalStatus(results, goal) {
...
exports.AutoBrowseExecutor = AutoBrowseExecutor;
exports.autoBrowseExecutor = new AutoBrowseExecutor();
