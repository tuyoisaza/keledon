...
export default function BrainPage() {
  ...
  const [sttLang, setSttLang] = useState(() => {
    try {
      return (
        localStorage.getItem("keledon_stt_lang") ||
        navigator.language ||
        "en-US"
      );
    } catch {
  ...
  const [autoSpeak, setAutoSpeak] = useState(() => {
    try {
      return localStorage.getItem(AUTOSPEAK_KEY) !== "false";
    } catch {
      return true;
    }
  });
  ...
  const [selectedMicId, setSelectedMicId] = useState(() => {
    try {
      return localStorage.getItem("keledon_brain_mic_id") || "";
    } catch {
      return "";
    }
  });
  const [selectedSpeakerId, setSelectedSpeakerId] = useState(() => {
    try {
      return localStorage.getItem("keledon_brain_speaker_id") || "";
    } catch {
      return "";
    }
  });
  ...
  const audioQueueRef = useRef<
    { data: string; seq: string; format?: string }[]
  >([]);
  ...
  const speachesStopTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  async function resolveBrainCloudConfig(): Promise<{
    apiBase: string;
    wsBase: string;
  }> {
    if (cloudApiBaseRef.current && cloudWsBaseRef.current) {
      return {
        apiBase: cloudApiBaseRef.current,
        wsBase: cloudWsBaseRef.current,
      };
    }
    let apiBase = API_URL || "";
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!apiBase && config.ws_url)
          apiBase = String(config.ws_url)
            .replace(/^wss:/, "https:")
            .replace(/^ws:/, "http:");
      }
    } catch (e) {
      addLog(
        `[v${__APP_VERSION__ || "?"}] cloud-config lookup failed, using fallback: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
    if (
      (!apiBase || !wsBase) &&
      window.location.hostname === "keledon.tuyoisaza.com"
    ) {
      apiBase = apiBase || "https://keledonapi.tuyoisaza.com";
      wsBase = wsBase || "wss://keledonapi.tuyoisaza.com";
    }
    cloudApiBaseRef.current = apiBase;
    cloudWsBaseRef.current =
      wsBase || window.location.origin.replace(/^http/, "ws");
    addLog(
      `[v${__APP_VERSION__ || "?"}] Brain cloud endpoints: api=${cloudApiBaseRef.current || "(same-origin)"} ws=${cloudWsBaseRef.current}`,
    );
  ...
  async function fetchPipelineStatus(sessionId: string) {
    try {
      ...
      const res = await fetch(
        `${apiBase}/api/voice-provider/status/${encodeURIComponent(sessionId)}`,
        {
      ...
      setPipelineStatus({
        stt: data.stt
          ? {
              id: data.stt.id,
              health: data.stt.health,
              name: data.stt.name,
              detail: data.stt.detail,
            }
          : null,
        tts: data.tts
          ? {
              id: data.tts.id,
              health: data.tts.health,
              name: data.tts.name,
              detail: data.tts.detail,
            }
          : null,
        llm: data.llm
          ? {
              id: data.llm.id,
              health: data.llm.health,
              name: data.llm.name,
      ...
      addLog(
        `[VOICE] Provider status error: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  }
    ...
    async function loadContext() {
      ...
      try {
        ...
        const fallbackCompanyId =
          firstCompanyWithBrands?.id || companyList[0]?.id || "";
        const initialCompanyId =
          stored.companyId || user?.companyId || fallbackCompanyId;
        const companyBrands = brandList.filter(
          (b) => b.companyId === initialCompanyId,
        );
      ...
      .then((data) => {
        if (data) {
          ...
          setTtsApiKeySet(
            !!(
              data.ttsApiKey ||
              (data.ttsProvider === "speaches" ? data.speachesApiKey : null)
            ),
          );
          const sttKey =
            data.sttProvider === "deepgram"
              ? data.deepgramApiKey
              : data.sttProvider === "speaches"
                ? data.speachesApiKey
                : null;
          setSttKeySet(!!sttKey);
          addLog(
            `Provider config loaded: LLM=${data.llmProvider || "?"} (key=${!!llmKey}) TTS=${data.ttsProvider || "?"} (key=${!!(data.ttsApiKey || (data.ttsProvider === "speaches" ? data.speachesApiKey : null))}) STT=${data.sttProvider || "?"} (key=${!!sttKey})`,
          );
        } else {
  ...
  useEffect(() => {
    ...
    if (!brandsForCompany.some((b) => b.id === selectedBrandId)) {
      ...
      setSelectedTeamId(
        teams.filter((t) => t.brandId === next.id)[0]?.id || "",
      );
    }
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (
        !target.closest("[data-device-panel]") &&
        !target.closest("[data-device-btn]")
      ) {
  ...
  useEffect(() => {
    addLog(
      `[v${__APP_VERSION__ || "?"}] BrainPage mounted — web v${__APP_VERSION__ || "?"}`,
    );
    addLog(
      `[v${__APP_VERSION__ || "?"}]   API_URL=${API_URL} | WEBSOCKET_URL=${WEBSOCKET_URL || "(same origin)"}`,
    );
    return () => {
    ...
    try {
      ...
      processor.onaudioprocess = (event) => {
        ...
        if (elapsedMs - lastLog > 1200) {
          lastLog = elapsedMs;
          addLog(
            `[BARGE-IN] monitoring during ${label}… level=${rms.toFixed(3)} hot=${hotFrames}`,
          );
        }
        if (hotFrames >= framesToTrigger && elapsedMs > 250) {
          addLog(
            `[BARGE-IN] user speech detected during ${label} → interrupting TTS (level=${rms.toFixed(3)})`,
          );
          stopBargeInMonitor();
      ...
      bargeInActiveRef.current = false;
      addLog(
        `[BARGE-IN] monitor unavailable: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }
  ...
  async function playWavPcmViaWebAudio(
    bytes: Uint8Array,
    label: string,
    onEnded: () => void,
  ): Promise<boolean> {
    try {
      ...
      while (pos + 8 <= bytes.length) {
        ...
        const chunkSize =
          declaredSize === 0xffffffff || declaredSize > remaining
            ? remaining
            : declaredSize;
      ...
      if (
        ...
        addLog(
          `[${label}] WebAudio WAV parser unsupported fmt=${audioFormat} channels=${channels} rate=${sampleRate} bits=${bitsPerSample} data=${dataSize}`,
        );
        return false;
      }
      ...
      addLog(
        `[${label}] WebAudio WAV playback start channels=${channels} rate=${sampleRate} samples=${sampleCount}`,
      );
      ...
      addLog(
        `[${label}] WebAudio WAV playback failed: ${err instanceof Error ? err.message : String(err)}`,
      );
      return false;
    }
  }
  ...
  function playBlobViaAudioElement(
    ...
    void audio.play().catch((err) => {
      addLog(
        `[${label}] Audio element play() rejected: ${err instanceof Error ? err.message : String(err)}`,
      );
      markTtsPlaybackEnded();
      URL.revokeObjectURL(url);
      onFailed();
  ...
  async function connectVoiceSocket() {
    ...
    addLog(
      `[v${appVersion}] Session: ${sessionId} | User: ${user?.email || user?.id || "anon"}`,
    );
    addLog(
      `[v${appVersion}] Auth token: ${token ? token.substring(0, 12) + "..." : "MISSING!"}`,
    );
    ...
    socket.on("connect", () => {
      ...
      setTransportStatus({
        mode: "initializing",
        detail: "Negotiating WebRTC offer/answer",
      });
      addLog(
        "[VOICE] Transport negotiation: attempting WebRTC; will fall back to WS audio if unavailable",
      );
      void setupWebRtc().then((ok) => {
        if (ok) {
          addLog(
            "[VOICE] WebRTC SDP negotiation complete; waiting for ICE/peer connected before selecting media channel",
          );
        } else {
          setTransportStatus({
            mode: "ws-fallback",
            detail: "WebRTC unavailable; using existing WS audio path",
          });
    ...
    socket.on("voice:brain:thinking", (data: any) => {
      addLog(
        `[v${__APP_VERSION__ || "?"}] Brain thinking "${data.text?.slice(0, 60)}"`,
      );
    });
    socket.on("voice:brain:reply", (data: any) => {
      addLog(
        `[v${__APP_VERSION__ || "?"}] Brain reply: ${data.text?.slice(0, 120)} | API v${data.apiVersion || "?"}`,
      );
      setSending(false);
    ...
    socket.on(
      ...
      (data: {
        ...
        // Cancel browser SpeechSynthesis if active — premium Speaches audio arrived
        if (data.audio && window.speechSynthesis?.speaking) {
          addLog(
        ...
        if (data.sequence === "end") {
          ...
          addLog(
            "Audio stream end" +
              ...
              (data.voice || data.language || data.ttsElapsedMs
                ? ` voice=${data.voice || "?"} lang=${data.language || "?"} tts=${data.ttsElapsedMs ?? "?"}ms`
                : "") +
          ...
          if (
            ...
            !lastChunkPlayedRef.current
          ) {
            // Don't restart if the progressive enhancement timer already started it
            if (audioPlayingRef.current) return;
            addLog(
              `[v${__APP_VERSION__ || "?"}] No audio from backend — using browser SpeechSynthesis`,
            );
    ...
    socket.on("voice:error", (data: any) => {
      addLog(
        `[v${__APP_VERSION__ || "?"}] WS ERROR: ${data.error || "unknown"}`,
      );
      setSending(false);
      audioPlayingRef.current = false;
      audioQueueRef.current = [];
    ...
    socket.on("voice:rpa:steps", async (data: { steps: RpaStep[] }) => {
      ...
      addLog(
        `[v${__APP_VERSION__ || "?"}] RPA: executing ${data.steps.length} steps`,
      );
      try {
        const result = await executeRpaSteps(data.steps);
        const okCount = result.results.filter((r) => r.success).length;
        addLog(
          `[v${__APP_VERSION__ || "?"}] RPA: ${okCount}/${result.results.length} OK`,
        );
        // Send result back to gateway
        voiceSocketRef.current?.emit("voice:rpa:result", result);
      } catch (err) {
        addLog(
          `[v${__APP_VERSION__ || "?"}] RPA: error — ${err instanceof Error ? err.message : String(err)}`,
        );
    ...
    socket.on("disconnect", (reason: string) => {
      ...
      addLog(
        `[v${__APP_VERSION__ || "?"}]   Session: ${voiceSessionIdRef.current || "none"} | Duration: ${formatCallTime(callTimer)}`,
      );
    ...
    socket.on("connect_error", (err) => {
      ...
      addLog(
        `[v${__APP_VERSION__ || "?"}]   Check: CORS? API running? URL=${fullUrl}`,
      );
      setCallStatus("disconnected");
  ...
  async function setupWebRtc(): Promise<boolean> {
    ...
    if (!socket?.connected) {
      setTransportStatus({
        mode: "ws-fallback",
        detail:
          "WS connected but signaling socket was not ready for WebRTC setup",
      });
    ...
    try {
      ...
      pc.oniceconnectionstatechange = () => {
        ...
        if (
          ...
          addLog(
            `[VOICE] Transport fallback active: ICE ${pc.iceConnectionState}, continuing over WS audio`,
          );
        }
      };
      pc.onconnectionstatechange = () => {
        ...
        if (pc.connectionState === "failed") {
          ...
          addLog(
            "[VOICE] Transport fallback active: WebRTC peer failed, continuing over WS audio",
          );
        }
      };
      ...
      if (response.error) {
        ...
        setTransportStatus({
          mode: "ws-fallback",
          detail: `Offer rejected: ${response.error}`,
        });
        pc.close();
      ...
      addLog(
        `[WebRTC] SDP answer accepted — ICE=${pc.iceConnectionState} peer=${pc.connectionState}`,
      );
      return true;
    } catch (err) {
  ...
  function disconnectVoiceSocket() {
    ...
    if (peerConnectionRef.current) {
      ...
      setTransportStatus({
        mode: "disconnected",
        detail: "Call disconnected; WebRTC peer closed",
      });
  ...
  function playNextAudioChunk() {
    ...
    try {
      ...
      addLog(
        `[VOICE] Backend audio chunk received seq=${chunk.seq} format=${isWav ? "wav" : "mpeg"} bytes=${bytes.length}`,
      );
      void playAudioBytes(
        bytes,
  ...
  function toggleAutoSpeak() {
    ...
    setAutoSpeak(next);
    try {
      localStorage.setItem(AUTOSPEAK_KEY, String(next));
    } catch {
      /* ignore */
    }
  ...
  function appendSpeachesTranscriptionFields(formData: FormData): {
    language: "es" | "en";
    model: string;
  } {
  ...
  function floatTo16BitPcmBase64(
    input: Float32Array,
  ...
  function makePcmWavBlob(
    chunks: Float32Array[],
    inputSampleRate: number,
    outputSampleRate = 16000,
  ): Blob {
    const totalInputSamples = chunks.reduce(
  ...
  async function startVoskListening(): Promise<void> {
    ...
    addLog(
      `[v${appVersion}] STT provider=vosk → starting Railway Vosk session`,
    );
    ...
    if (!sessionRes.ok) {
      ...
      throw new Error(
        `Vosk session create failed ${sessionRes.status}: ${body}`,
      );
    }
    const session = await sessionRes.json();
    const sessionId = session.sessionId;
    const language = (sttLangRef.current || navigator.language || "en")
      .toLowerCase()
      .startsWith("es")
      ? "es"
      : "en";
    ...
    addLog(
      `[v${appVersion}] Connecting Vosk WS → ${listenUrl} path=/listen/ws session=${sessionId}`,
    );
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      listenSocket.on("connect", async () => {
        addLog(
          `[v${appVersion}] Vosk WS connected: ${listenSocket.id} | session=${sessionId} | lang=${language}`,
        );
        try {
          const stream = await navigator.mediaDevices.getUserMedia({
            audio: selectedMicId
          ...
          processor.onaudioprocess = (event) => {
            ...
            const input = event.inputBuffer.getChannelData(0);
            const payload = floatTo16BitPcmBase64(
              input,
              audioCtx.sampleRate,
              16000,
            );
      ...
      listenSocket.on("session.ended", (data: any) => {
        ...
        clearTimeout(failSafe);
        if (
          reason.includes("worker_error") &&
          reason.includes(VOSK_FAIL_REASON)
        ) {
          if (!rejected) {
            rejected = true;
            stopVoskListening();
  ...
  function stopVoskListening(): void {
    ...
    if (listenSocketRef.current) {
      if (listenSocketRef.current.connected)
        listenSocketRef.current.emit("session.stop");
  ...
  function stopSpeachesProvisionalRecognition() {
    ...
    if (provisional) {
      try {
        provisional.onresult = null;
        provisional.onerror = null;
        provisional.onend = null;
        provisional.stop();
      } catch {
        /* ignore */
      }
  ...
  function startSpeachesProvisionalRecognition() {
    ...
    try {
      ...
      provisional.onerror = (event: any) =>
        addLog(
          "[STT] Provisional transcript ended: " + (event?.error || "unknown"),
        );
      provisional.onend = () => {
        if (
          listeningRef.current &&
      ...
      provisional.start();
      addLog(
        "[STT] Provisional transcript: ON (Speaches final remains source of truth)",
      );
    } catch (err) {
      addLog(
        "[STT] Provisional transcript start failed: " +
    ...
    try {
      ...
      processor.onaudioprocess = (event) => {
        ...
        if (elapsedMs - lastLevelLog > 1200) {
          ...
          addLog(
            `[STT] Speaches listening… level=${rms.toFixed(3)} heard=${heardSpeech ? "yes" : "no"}`,
          );
        }
        if (
          heardSpeech &&
          elapsedMs > minListenMs &&
          silenceMs >= silenceStopMs
        ) {
          addLog(
            `[STT] Speaches VAD: ${Math.round(silenceMs)}ms silence → transcribe now`,
          );
          stopSpeachesListening();
        } else if (elapsedMs >= maxListenMs) {
          addLog(
            "[STT] Speaches VAD: max turn length reached → transcribe now",
          );
          stopSpeachesListening();
        }
      };
  ...
  async function startSpeachesListening() {
    const appVersion = __APP_VERSION__ || "?";
    addLog(
      `[v${appVersion}] STT provider=speaches → starting fast turn capture via Speaches`,
    );
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        audio: selectedMicId ? { deviceId: { exact: selectedMicId } } : true,
      ...
      if (
        ...
        addLog(
          `[WebRTC] mic track attached via replaceTrack; ICE=${pc.iceConnectionState} peer=${pc.connectionState}`,
        );
        setTransportStatus((prev) => ({
          ...prev,
        ...
        addLog(
          "[VOICE] Audio capture path: WS fallback/recorder path active for this turn",
        );
      }
      ...
      recorder.onstop = async () => {
        ...
        if (activeSender) {
          void activeSender.replaceTrack(null).catch((err) => {
            addLog(
              `[WebRTC] mic track detach failed: ${err instanceof Error ? err.message : String(err)}`,
            );
          });
        }
        ...
        if (
          conversationModeRef.current &&
          provisional &&
          !speachesFastSentRef.current
        ) {
          speachesFastSentRef.current = true;
          setDraft(provisional);
          draftRef.current = provisional;
          addLog(
            `[VOICE] Fast path: sending provisional transcript to Brain now: "${provisional.substring(0, 80)}${provisional.length > 80 ? "..." : ""}"`,
          );
          if (voiceSocketRef.current?.connected)
            sendVoiceTranscript(provisional);
          else void handleSend(provisional);
        }
        addLog(
          `[STT] Speaches: audio captured, transcribing final${provisional ? ` (provisional="${provisional.substring(0, 80)}${provisional.length > 80 ? "..." : ""}")` : ""}...`,
        );
        const wavBlob = makePcmWavBlob(
          speachesPcmChunksRef.current,
          speachesPcmSampleRateRef.current,
          16000,
        );
        addLog(
          `[STT] Speaches: sending WAV PCM ${wavBlob.size} bytes @16kHz to transcription proxy`,
        );
        ...
        try {
          ...
          addLog(
            `[STT] Speaches proxy config resolved url=${String(_apiUrl).replace(/\/+$/, "")} key=${_apiKey ? "yes" : "no"}`,
          );
          ...
          addLog(
            `[STT] Speaches final request lang=${sttRequest.language} model=${sttRequest.model}`,
          );

          const res = await apiFetch(
            `/api/teams/${selectedTeamId}/speaches/transcriptions`,
            {
              method: "POST",
              body: formData,
            },
          ...
          if (res.ok) {
            ...
            const text = (data.text || "").trim();
            addLog(
              `[TEST] STT: ✅ Speaches final transcript: "${text.substring(0, 80)}${text.length > 80 ? "..." : ""}"`,
            );
            if (text) {
              const normalizedTextForNoise = text
                .toLowerCase()
                .replace(/[^a-záéíóúñü¿¡\s]/gi, "")
                .replace(/\s+/g, " ")
                .trim();
              const postPlaybackEcho =
                Date.now() < postPlaybackEchoGuardUntilRef.current;
              const lowInfoFinalOnly =
                !provisional &&
                (normalizedTextForNoise.length < 6 ||
                  ["so", "yeah", "ok", "okay", "uh", "um", "eh", "ah"].includes(
                    normalizedTextForNoise,
                  ));
              if (
                lowInfoFinalOnly ||
                (postPlaybackEcho &&
                  !provisional &&
                  normalizedTextForNoise.split(" ").length <= 2)
              ) {
                addLog(
                  `[STT] Speaches final looks like playback echo/noise; skipping auto-send (final="${text.substring(0, 80)}")`,
                );
                stream.getTracks().forEach((t) => t.stop());
                listeningRef.current = false;
                setIsListening(false);
              ...
              if (speachesFastSentRef.current) {
                const normalizedFinal = text
                  .toLowerCase()
                  .replace(/\s+/g, " ")
                  .trim();
                const normalizedProvisional = provisional
                  .toLowerCase()
                  .replace(/\s+/g, " ")
                  .trim();
                if (
                  normalizedFinal &&
                  normalizedFinal !== normalizedProvisional
                ) {
                  addLog(
                    `[STT] Speaches final differed from fast provisional; not re-sending this turn (final="${text.substring(0, 80)}${text.length > 80 ? "..." : ""}")`,
                  );
                } else {
            ...
            addLog(
              `[TEST] STT: ❌ Speaches HTTP ${res.status}: ${err.substring(0, 200)}`,
            );
          }
        } catch (err) {
          addLog(
            `[TEST] STT: ❌ Speaches error: ${err instanceof Error ? err.message : String(err)}`,
          );
        }
      ...
      addLog(
        `[v${appVersion}] Speaches: ❌ could not start: ${err instanceof Error ? err.message : String(err)}`,
      );
  ...
  async function toggleListening() {
    ...
    if (sttProvider === "vosk") {
      try {
        ...
        addLog(
          `[v${__APP_VERSION__ || "?"}] Vosk unavailable, falling back to browser SpeechRecognition: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
    ...
    recognition.onstart = () => {
      addLog(
        `[v${__APP_VERSION__ || "?"}] STT: lang=${recognition.lang}, continuous=${recognition.continuous}, interim=${recognition.interimResults}`,
      );
    ...
    recognition.onresult = (event: any) => {
      ...
      if (final) {
        ...
        addLog('STT final: "' + final.trimEnd() + '"');
        ...
        setDraft(interim);
        draftRef.current = interim;
      }
    };

    recognition.onend = () => {
      addLog(
        "recognition onend, draftRef=" +
          (draftRef.current
    ...
    try {
      ...
      addLog("recognition start error: " + e);
      listeningRef.current = false;
    }
  }
  ...
  function toggleConversationMode() {
    ...
    addLog(
      `[v${__APP_VERSION__ || "?"}]   Context: ${selectedCompany?.name || "-"} / ${selectedBrand?.name || "-"} / ${selectedTeam?.name || "-"}`,
    );
    if (conversationMode) {
  ...
  const [llmTestStatus, setLlmTestStatus] =
    useState<ProviderTestStatus>("idle");
  const [ttsTestStatus, setTtsTestStatus] =
    useState<ProviderTestStatus>("idle");
  const [sttTestStatus, setSttTestStatus] =
    useState<ProviderTestStatus>("idle");
  ...
  async function testLLMProvider() {
    ...
    try {
      ...
      addLog(
        `[TEST] LLM: response (${reply.length} chars): "${reply.substring(0, 120)}"`,
      );
      if (reply.toLowerCase().includes("working")) {
        setLlmTestStatus("pass");
        addLog(
          `[TEST] LLM: PASS ✅ reply contains "WORKING" - configured provider is responding`,
        );
      } else if (reply.startsWith("I understand")) {
        setLlmTestStatus("fail");
        addLog(
          `[TEST] LLM: FAIL ❌ fallback response detected - configured provider did not answer`,
        );
      } else {
      ...
      setLlmTestStatus("fail");
      addLog(
        `[TEST] LLM: FAIL ❌ ${err instanceof Error ? err.message : String(err)}`,
      );
    } finally {
      setTestingLLM(false);
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (blob.size > 200) {
          ...
          addLog(
            `[TEST] TTS: FAIL ❌ empty/small audio blob (${blob.size} bytes)`,
          );
        }
      } else {
        ...
        setTtsTestStatus("fail");
        addLog(
          `[TEST] TTS: FAIL ❌ HTTP ${res.status}: ${text.substring(0, 300)}`,
        );
      }
    } catch (err: any) {
      if (err?.name === "AbortError") {
        ...
        setTtsTestStatus("fail");
        addLog(
          `[TEST] TTS: FAIL ❌ ${err instanceof Error ? err.message : String(err)}`,
        );
      }
    } finally {
      setTestingTTS(false);
    ...
    try {
      if (sttProvider === "vosk") {
        ...
        if (res.ok) {
          ...
          addLog(
            `[TEST] STT: FAIL ❌ Vosk session failed HTTP ${res.status}: ${text.substring(0, 300)}`,
          );
        }
        ...
        if (SR) {
          ...
          setSttTestStatus("fail");
          addLog(
            `[TEST] STT: FAIL ❌ Web Speech API not available in this browser`,
          );
        }
        ...
        const sttRequest = appendSpeachesTranscriptionFields(formData);
        addLog(
          `[TEST] STT: Speaches request lang=${sttRequest.language} model=${sttRequest.model}`,
        );
        const res = await apiFetch(
          `/api/teams/${selectedTeamId}/speaches/transcriptions`,
          {
        ...
        if (res.ok) {
          ...
          setSttTestStatus("pass");
          addLog(
            `[TEST] STT: PASS ✅ Speaches proxy transcription OK (${(data.text || "").substring(0, 80) || "no speech text from synthetic test audio"})`,
          );
        } else {
          const txt = await res.text().catch(() => "");
          setSttTestStatus("fail");
          addLog(
            `[TEST] STT: FAIL ❌ Speaches proxy HTTP ${res.status}: ${txt.substring(0, 300)}`,
          );
        }
      } else if (sttProvider) {
      ...
      setSttTestStatus("fail");
      addLog(
        `[TEST] STT: FAIL ❌ ${err instanceof Error ? err.message : String(err)}`,
      );
    } finally {
      setTestingSTT(false);
    }
  ...
  async function handleSend(overrideText?: string) {
    ...
    const nextHistory: BrainChatMessage[] = [...messages, userMessage].map(
      (m) => ({
        role: m.role,
        content: m.content,
      }),
        ...
        <div className="space-y-2">
          ...
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              Configure the company, brand, and team context, then chat with
              KELEDON Brain — by text or voice.
            </p>
          </div>
        </div>
                ...
                testStatus === "pass"
                  ...
                  : testStatus === "fail"
                    ...
                    : testStatus === "warn"
                      ? {
                          label: "WARN",
                          cls: "bg-yellow-500/10 text-yellow-300 border-yellow-500/20",
                          dot: "bg-yellow-300",
                        }
                      : testStatus === "running"
                        ? {
              ...
              return (
                <div className="flex items-center justify-between gap-2 px-2 py-1.5 rounded-md bg-muted/30 text-xs">
                  ...
                  <span className="flex items-center gap-1.5">
                    <span
                      className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium ${badgeClass}`}
                    >
                      {providerName || "none"}
                    </span>
                    <span
                    ...
                    >
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${keySet ? "bg-green-400" : "bg-red-400"}`}
                      />
              ...
              <div className="space-y-1 pt-2 border-t border-border">
                <div className="flex items-center justify-between px-1">
                  ...
                  <span className="flex items-center gap-2">
                    ...
                    <a
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        window.open("/management/providers", "_blank");
                      }}
                    >
                      edit →
                    </a>
                ...
                {teamConfigLoading ? (
                  ...
                  <div className="space-y-1">
                    ...
                    {ttsVoiceId && (
                      <div className="flex items-center justify-between px-2 py-1 text-[10px] text-muted-foreground">
                        <span>Voice ID</span>
                        <code className="px-1 py-0.5 rounded bg-muted/50 font-mono">
                          {ttsVoiceId}
                        </code>
                      </div>
                    )}
                  ...
                  <div className="px-2 py-1 text-[10px] text-muted-foreground">
                    Select a team to see provider config
                  </div>
                )}
              </div>
            );
          ...
          {callStatus === "connected" && pipelineStatus && (
            <div className="space-y-1 pt-2 border-t border-border mt-2">
              <div className="flex items-center justify-between px-1">
                ...
                <span
                  className={`h-1.5 w-1.5 rounded-full ${callStatus === "connected" ? "bg-green-400 animate-pulse" : "bg-muted-foreground"}`}
                />
              </div>
              <div className="space-y-1">
                {(() => {
                  const transportMeta: Record<
                    typeof transportStatus.mode,
                    { label: string; cls: string; dot: string; value: string }
                  > = {
                    webrtc: {
                      label: "WEBRTC",
                      value: "media channel",
                    ...
                    <div className="rounded-md bg-muted/20 px-2 py-1 text-[10px]">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="flex items-center gap-1.5">
                          ...
                          <span
                            className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded border ${meta.cls}`}
                          >
                            <span
                      ...
                      {(transportStatus.iceState ||
                        transportStatus.peerState ||
                        transportStatus.detail) && (
                        <div
                          className="mt-0.5 truncate text-[9px] text-muted-foreground"
                          title={transportStatus.detail || ""}
                        >
                          ICE={transportStatus.iceState || "?"} · Peer=
                          {transportStatus.peerState || "?"}
                          {transportStatus.detail
                            ? ` · ${transportStatus.detail}`
                            : ""}
                ...
                {[pipelineStatus.llm, pipelineStatus.stt, pipelineStatus.tts]
                  .filter(Boolean)
                  .map((provider) => {
                    ...
                    const meta = healthMap[provider.health] || {
                      label: provider.health.toUpperCase(),
                      cls: "bg-muted/40 text-muted-foreground border-border",
                      dot: "bg-muted-foreground",
                    };
                    const kindLabel =
                      provider === pipelineStatus.llm
                        ? "LLM"
                        : provider === pipelineStatus.stt
                          ? "STT"
                          : "TTS";
                    return (
                      <div
                        key={kindLabel}
                        className="flex items-center justify-between px-2 py-1 rounded-md bg-muted/20 text-[10px]"
                      >
                        <span className="font-medium text-foreground">
                          {kindLabel}
                        </span>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-border bg-muted/30 p-4 text-sm text-muted-foreground">
                ...
                <p className="mt-2">
                  The chat will use the selected company, brand, and team as the
                  ...
                  >
                    <div className="space-y-3 text-sm">
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        Speaker selection works in Chrome/Edge when using TTS or
                        voice call audio.
                      </p>
                    </div>
                  </div>
                ...
                >
                  {callStatus === "connecting" && (
                    <div className="text-center">
                      ...
                      <p className="mt-3 text-base font-medium text-yellow-400">
                        Calling KELEDON Brain...
                      </p>
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    try {
                      localStorage.setItem("keledon_stt_lang", lang);
                    } catch {
                      /* ignore localStorage errors */
                    }
