    ...
    return this.tracer.startActiveSpan(
      ...
      async (span) => {
        ...
        try {
          ...
          if (!infoResponse.ok) {
            throw new Error(
              `Qdrant collection lookup failed: HTTP ${infoResponse.status}`,
            );
          }

          const infoJson: any = await infoResponse.json();
          const vectorSize = Number(
            infoJson?.result?.config?.params?.vectors?.size,
          );
          ...
          if (!searchResponse.ok) {
            ...
            throw new Error(
              `Qdrant search failed: HTTP ${searchResponse.status} ${detail}`,
            );
          }

          const searchJson: any = await searchResponse.json();
          const results: RetrievalResult[] = (searchJson?.result || []).map(
            (point: any) => {
              const payload = (point?.payload || {}) as Record<string, any>;
              const docId = String(
                payload.doc_id || payload.id || point?.id || '',
              );
