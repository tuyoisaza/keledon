...
export class VoiceAnalyticsService {
  ...
  async processTranscript(
    ...
    console.log(
      `VoiceAnalytics: Processed conversation ${conversationId}, sentiment: ${metrics.sentiment}`,
    );
    return metrics;
  }
  ...
  getAnalytics(
    ...
    return {
      ...
      sentimentDistribution:
        this.calculateSentimentDistribution(recentConversations),
  ...
  private analyzeSpeakerStats(segments: TranscriptSegment[]): SpeakerStats {
    ...
    const customerTime = customerSegments.reduce(
    ...
    return {
      totalSpeakTime:
        totalTime > 0 ? Math.round((customerTime / totalTime) * 100) : 50,
  ...
  private findLongestPause(segments: TranscriptSegment[]): number {
    ...
    for (let i = 1; i < segments.length; i++) {
      const pause =
        (segments[i].timestamp.getTime() -
          segments[i - 1].timestamp.getTime()) /
        1000;
  ...
  private analyzeQuality(segments: TranscriptSegment[]): QualityMetrics {
    ...
    const satisfaction = Math.round(
      Math.max(0, Math.min(100, (sentiment + 1) * 50)),
    );
  ...
  private extractKeywords(segments: TranscriptSegment[]): KeywordAnalysis[] {
    ...
    return Array.from(wordCounts.entries())
      ...
      .map(([word, count]) => ({
        ...
        sentiment: this.positiveWords.has(word)
          ? 'positive'
  ...
  private calculateSuccessRate(conversations: ConversationMetrics[]): number {
    const successful = conversations.filter(
      (conv) => conv.qualityMetrics.satisfaction > 60,
    ).length;
  ...
  private aggregateSpeakerStats(conversations: ConversationMetrics[]): {
    customer: SpeakerStats;
    agent: SpeakerStats;
  } {
    const customerStats = {
      totalSpeakTime: Math.round(
        conversations.reduce(
          (sum, conv) => sum + conv.speakerStats.totalSpeakTime,
          0,
        ) / conversations.length,
      ),
      avgSpeakingRate: Math.round(
        conversations.reduce(
          (sum, conv) => sum + conv.speakerStats.avgSpeakingRate,
          0,
        ) / conversations.length,
      ),
      interruptions: Math.round(
        conversations.reduce(
          (sum, conv) => sum + conv.speakerStats.interruptions,
          0,
        ) / conversations.length,
      ),
      longestPause: Math.round(
        conversations.reduce(
          (sum, conv) => sum + conv.speakerStats.longestPause,
          0,
        ) / conversations.length,
      ),
      fillerWords: Math.round(
        conversations.reduce(
          (sum, conv) => sum + conv.speakerStats.fillerWords,
          0,
        ) / conversations.length,
      ),
    };
  ...
  private aggregateQualityMetrics(
    ...
    return {
      clarity: Math.round(
        conversations.reduce(
          (sum, conv) => sum + conv.qualityMetrics.clarity,
          0,
        ) / conversations.length,
      ),
      completeness: Math.round(
        conversations.reduce(
          (sum, conv) => sum + conv.qualityMetrics.completeness,
          0,
        ) / conversations.length,
      ),
      relevance: Math.round(
        conversations.reduce(
          (sum, conv) => sum + conv.qualityMetrics.relevance,
          0,
        ) / conversations.length,
      ),
      satisfaction: Math.round(
        conversations.reduce(
          (sum, conv) => sum + conv.qualityMetrics.satisfaction,
          0,
        ) / conversations.length,
      ),
    };
  ...
  cleanup(): void {
    ...
    if (cleanedCount > 0) {
      console.log(
        `VoiceAnalytics: Cleaned up ${cleanedCount} old conversations`,
      );
    }
  }
}
