...
export class DashboardController {
    ...
    try {
      ...
      if (timeRange.from) {
        ...
        filteredLoops = filteredLoops.filter(
          (loop) => loop.startTime && loop.startTime >= fromDate,
        );
      }
  ...
  async getVoiceAnalytics(@Query() timeRange: TimeRange) {
    try {
      ...
      console.error(
        'DashboardController: Error getting voice analytics',
        error,
      );
      throw new HttpException(
    ...
    try {
      ...
      return {
        ...
        data: integrations.map((provider) => ({
          ...
          lastSync:
            provider.lastSync?.toISOString() || new Date().toISOString(),
      ...
      console.error(
        'DashboardController: Error getting integration health',
        error,
      );
      throw new HttpException(
  ...
  async getFlowExecutions(@Query() timeRange: TimeRange) {
    try {
      ...
      console.error(
        'DashboardController: Error getting flow executions',
        error,
      );
      throw new HttpException(
        'Failed to get flow executions',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getDashboardStatus() {
    try {
      ...
      console.error(
        'DashboardController: Error getting dashboard status',
        error,
      );
      throw new HttpException(
