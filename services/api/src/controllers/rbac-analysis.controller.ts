...
import {
  ...
  RBACAnalysis,
  RBACAnalysisFilter,
} from '../services/rbac-recommendation.service';

@ApiTags('RBAC Analysis')
@Controller('rbac-analysis')
export class RBACAnalysisController {
  constructor(
    private readonly rbacRecommendationService: RBACRecommendationService,
  ) {}
  ...
  @ApiResponse({
    status: 200,
    description: 'RBAC analysis completed successfully',
  })
  @ApiResponse({ status: 400, description: 'Invalid request parameters' })
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async performAnalysis(@Body() request?: any): Promise<{
    success: boolean;
    data: RBACAnalysis;
    message: string;
  }> {
    try {
      console.log(
        'RBACAnalysisController: Starting comprehensive RBAC analysis',
        {
      ...
      const analysis =
        await this.rbacRecommendationService.performFullAnalysis(request);
      ...
      throw new HttpException(
        'Failed to perform RBAC analysis',
  ...
  async getAnalysesHistory(
    ...
    @Query('sort') _sort?: 'date' | 'score' | 'risk',
    @Query('order') _order?: 'asc' | 'desc',
  ): Promise<{
    success: boolean;
    data: {
    ...
    try {
      const analyses =
        await this.rbacRecommendationService.getAnalysisHistory();

      const totalGaps = analyses.reduce(
        (sum, analysis) => sum + analysis.gaps.length,
        0,
      );
      ...
      const avgScore =
        analyses.length > 0
          ? analyses.reduce(
              (sum, analysis) => sum + analysis.maturityScore,
              0,
            ) / analyses.length
          : 0;
      const avgRiskLevel =
        analyses.length > 0
          ? analyses
              .map((a) => a.riskAssessment.overallRisk)
              .sort((a, b) => {
                const riskOrder = { low: 1, medium: 2, high: 3, critical: 4 };
                return riskOrder[a] - riskOrder[b];
              })[Math.floor(analyses.length / 2)] || 'medium'
          : 'medium';
      ...
      console.error(
        'RBACAnalysisController: Error getting analysis history',
        error,
      );
      throw new HttpException(
  ...
  async getAnalysisById(@Param('id') id: string): Promise<{
    ...
    try {
      ...
      console.error(
        'RBACAnalysisController: Error getting analysis by ID',
        error,
      );
      throw new HttpException(
  ...
  async getAnalysisGaps(@Query() filters?: RBACAnalysisFilter): Promise<{
    success: boolean;
    data: any[];
    ...
    try {
      const gaps =
        await this.rbacRecommendationService.getAnalysisGaps(filters);

      const severity = gaps.reduce(
        (acc, gap) => {
      ...
      console.error(
        'RBACAnalysisController: Error getting analysis gaps',
        error,
      );
      throw new HttpException(
        'Failed to get analysis gaps',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getAnalysisRecommendations(
    ...
    try {
      const recommendations =
        await this.rbacRecommendationService.getRecommendations(filters);

      const priorities = recommendations.reduce(
        (acc, rec) => {
      ...
      const categories = recommendations.reduce(
        (acc, rec) => {
      ...
      console.error(
        'RBACAnalysisController: Error getting analysis recommendations',
        error,
      );
      throw new HttpException(
        'Failed to get analysis recommendations',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Implementation plan retrieved successfully',
  })
  async getImplementationPlan(): Promise<{
    ...
    try {
      ...
      console.error(
        'RBACAnalysisController: Error getting implementation plan',
        error,
      );
      throw new HttpException(
        'Failed to get implementation plan',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async generateReport(@Param('analysisId') analysisId: string): Promise<{
    ...
    try {
      const reportBuffer =
        await this.rbacRecommendationService.generateReport(analysisId);
  ...
  async getDashboardData(): Promise<{
    ...
    try {
      const dashboardData =
        await this.rbacRecommendationService.getDashboardData();
      return {
        success: true,
        data: dashboardData,
      };
      ...
      console.error(
        'RBACAnalysisController: Error getting dashboard data',
        error,
      );
      throw new HttpException(
  ...
  async getAIInsights(): Promise<{
    ...
    try {
      const insights =
        await this.rbacRecommendationService.generateAIInsights();
      return {
        success: true,
        data: insights,
      };
      ...
      console.error(
        'RBACAnalysisController: Error generating AI insights',
        error,
      );
      throw new HttpException(
