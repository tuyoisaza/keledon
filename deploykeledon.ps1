...
if ([string]::IsNullOrEmpty($ProjectId)) {
    Write-Host "No project specified, trying to detect..."
    try {
        $ProjectId = gcloud config list project --format='value(core.project)' 2>$null
        if ([string]::IsNullOrEmpty($ProjectId)) {
            Write-Host "No project configured, listing available projects..."
            ...
            if ($projects -and $projects.Count -gt 0) {
                ...
                Write-Host "ERROR: No projects available." -ForegroundColor Red
                exit 1
            }
        }
    } catch {
        Write-Host "ERROR: Cannot access projects. Enable Cloud Resource Manager API." -ForegroundColor Red
        exit 1
    }
}
...
Write-Host "   Update deployment: .\deploykeledon.ps1" -ForegroundColor Gray
