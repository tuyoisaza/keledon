...
import { RBACRecommendationService, RBACAnalysis, RBACAnalysisFilter } from '../services/rbac-recommendation.service';
...
export class RBACAnalysisController {
  ...
  async getAnalysesHistory(
    ...
    try {
      ...
      const avgRiskLevel = analyses.length > 0 ? 
        analyses.map(a => a.riskAssessment.overallRisk).sort((a, b) => {
          const riskOrder = { 'low': 1, 'medium': 2, 'high': 3, 'critical': 4 };
          return riskOrder[a as keyof typeof riskOrder] - riskOrder[b as keyof typeof riskOrder];
        })[Math.floor(analyses.length / 2)] || 'medium' : 'medium';
