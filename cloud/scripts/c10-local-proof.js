...
function assertDevOnly() {
  if (process.env.NODE_ENV === 'production') {
    fail('NODE_ENV=production blocked. C10 proof is DEV-ONLY.');
  }
...
function seedQdrant() {
  ...
  runOrFail('node', cmd, { cwd: process.cwd() });
}
  ...
  while (Date.now() < deadline) {
    ...
    const candidate = spans
      ...
      .find(
        ({ tags }) =>
          ...
          String(tags['execution_result']) === 'success' &&
    ...
    if (candidate) {
      return {
        ...
        execution_result: String(candidate.tags['execution_result'] || 'unknown'),
      };
    }
...
function assertEvidence(evidence, results) {
  ...
  const execTags = tagsAsMap(agentExecSpan);
  if (String(execTags['trace_id']) !== String(evidence.trace_id)) {
    fail('agent.exec trace_id does not match extension evidence trace_id.');
  }

  if (String(execTags['execution_result']) !== 'success') {
    fail('agent.exec execution_result is not success.');
  }
}
...
async function main() {
  ...
  console.log('[C10-PROOF] DEV-ONLY bootstrap: ensuring local DB tables exist...');
  bootstrapLocalPostgres();
  seedQdrant();

  console.log('[C10-PROOF] Building cloud service...');
  runNpmOrFail(['run', 'build'], { cwd: process.cwd() });

  console.log('[C10-PROOF] Building agent extension...');
  ...
  try {
    ...
    console.log('\n[C10-PROOF][MANUAL] Real extension runtime checkpoint required.');
    console.log('[C10-PROOF][MANUAL] 1) Load unpacked extension from agent/dist in Chrome');
    console.log(`[C10-PROOF][MANUAL] 2) In side panel set Cloud Runtime URL to http://localhost:${proofPort} via Backend Config`);
    console.log('[C10-PROOF][MANUAL] 3) Open side panel debug menu');
    console.log('ACTION REQUIRED: Click START LISTENING in Chrome Extension');
    console.log('[C10-PROOF][MANUAL] 4) Click "START LISTENING (C10)" in debug menu');
    console.log(`[C10-PROOF][MANUAL] Waiting up to ${Math.floor(EXTENSION_WAIT_MS / 1000)}s for keledon.agent.exec...`);
    ...
    console.log('\n[C10-PROOF][SUCCESS] Real extension runtime proof completed.');
    console.log(`[C10-PROOF] decision_id=${extensionEvidence.decision_id}`);
    console.log(`[C10-PROOF] trace_id=${extensionEvidence.trace_id}`);
    console.log(`[C10-PROOF] qdrant_collection=${QDRANT_COLLECTION}`);
    console.log('[C10-PROOF] qdrant_doc_ids=doc-1,doc-2,doc-3');
    console.log(`[C10-PROOF] jaeger_query=${results['keledon.command.emit'].url}`);
    console.log(
      `[C10-PROOF] extension_execution_log=event:agent.exec.end command_type:${extensionEvidence.command_type} tab_id:${extensionEvidence.tab_id} execution_result:${extensionEvidence.execution_result}`,
    );
  } catch (error) {
